//
//  UIControl+ClickInterval.m
//  DemoYiXinLi
//
//  Created by kangkang zhu on 2020/4/28.
//  Copyright © 2020 KangkangZhu. All rights reserved.
//

#import "UIControl+ClickInterval.h"
#import <objc/runtime.h>

static double kDefaultInterval = 0.5;

@interface UIControl ()
@property (nonatomic, assign) BOOL isIgnoreClick;
@property (strong, nonatomic) NSMutableArray *arrsSELName;
@property (strong, nonatomic) NSMutableArray *arrsControl;
@end

@implementation UIControl (ClickInterval)

+ (void)exchangeClickMethod {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        SEL originalSel = @selector(sendAction:to:forEvent:);
        SEL newSel = @selector(sendClickIntervalAction:to:forEvent:);
        Method originalMethod = class_getInstanceMethod(self , originalSel);
        Method newMethod = class_getInstanceMethod(self , newSel);
        //   如果发现方法已经存在，返回NO；也可以用来做检查用,我们这里是为了避免源方法没有实现的情况;如果方法没有存在,我们则先尝试添加被替换的方法的实现
        BOOL isAddNewMethod = class_addMethod(self, originalSel, method_getImplementation(newMethod), "v@:");
        if (isAddNewMethod) {
            class_replaceMethod(self, newSel, method_getImplementation(originalMethod), "v@:");
        } else {
            method_exchangeImplementations(originalMethod, newMethod);
        }
    });
}

- (void)sendClickIntervalAction:(SEL)action to:(id)target forEvent:(UIEvent *)event {
    if ([self isKindOfClass:[UIButton class]]) {
        if (self.clickInterval <= 0) {
            self.clickInterval = kDefaultInterval;
        };
        
        NSString *currentSELName = NSStringFromSelector(action);
        if (self.isIgnoreClick
            && [self.arrsSELName containsObject:currentSELName]
            && [self.arrsControl containsObject:self]) {
            return;
        }
        if (self.arrsSELName.count == 0) {
            self.arrsSELName = [NSMutableArray arrayWithObject:currentSELName];
        } else {
            [self.arrsSELName addObject:currentSELName];
        }
        
        if (self.arrsControl.count == 0) {
            self.arrsControl = [NSMutableArray arrayWithObject:self];
        } else {
            [self.arrsControl addObject:self];
        }
        
        if (self.clickInterval > 0) {
            self.isIgnoreClick = YES;
            [self performSelector:@selector(kk_ignoreClickState:)
                       withObject:@(NO)
                       afterDelay:self.clickInterval];
        }
    }
    [self sendClickIntervalAction:action to:target forEvent:event];
}

- (void)kk_ignoreClickState:(NSNumber *)ignoreClickState {
    self.isIgnoreClick = ignoreClickState.boolValue;
    [self.arrsSELName removeAllObjects];
    [self.arrsControl removeAllObjects];
}

- (NSTimeInterval)clickInterval {
    return [objc_getAssociatedObject(self, _cmd) doubleValue];
}

- (void)setClickInterval:(NSTimeInterval)clickInterval {
    objc_setAssociatedObject(self, @selector(clickInterval), @(clickInterval), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (BOOL)isIgnoreClick {
    return [objc_getAssociatedObject(self, _cmd) boolValue];
}

- (void)setIsIgnoreClick:(BOOL)isIgnoreClick {
    objc_setAssociatedObject(self, @selector(isIgnoreClick), @(isIgnoreClick), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (NSMutableArray *)arrsControl {
    return objc_getAssociatedObject(self, _cmd);
}

-(void)setArrsControl:(NSMutableArray *)arrsControl {
    objc_setAssociatedObject(self, @selector(arrsControl), arrsControl, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

-(NSMutableArray *)arrsSELName {
    return objc_getAssociatedObject(self, _cmd);
}
-(void)setArrsSELName:(NSMutableArray *)arrsSELName {
    objc_setAssociatedObject(self, @selector(arrsSELName), arrsSELName, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

@end
